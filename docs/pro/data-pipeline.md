...
Pro 的代码与历史记录涉及真实轨迹清洗和目标能力缺口合成两类工作。下面是项目层面的通用流程，**不表示 v2 / pc8 实际混入了直接清洗后的线上轨迹**。

针对这次实验，构建记录说明 v2 由老合成 PASS 池选出的 681 条和新缺口轨迹 468 条组成，再删除 92 条得到 v2.1。线上数据主要提供需求种子：先回查历史补齐 query，再构造环境、重新执行并收集 PASS。逐阶段输入 / 输出、模型分工、seed 准备与上传、真实 Rollout、PASS 收割和监督格式见[数据合成 Pipeline 详解](data-construction.md)。

建议阅读顺序：先看上述本批 Pipeline，再看[交互轮次与真实轨迹](sft-interactions.md)，最后用本文了解项目层面的通用方法与其他分支。
...
旧上传的两个代码快照不包含 `gen_gap_queries.py`；2026-09-16 已在本地 `lsy/post-train/tools/pro_chain_pipeline/` 找到并核对相关实现：复用原 world，仅替换 `query_zh` 和验收目标，seed 不改。脚本未整体公开，核查范围见[来源索引](sources.md#数据构造链路补充核查)。
...
4. 通过判官评价本轮任务推进与约束遵守，收集满足要求的轨迹；固定 v61 允许合理澄清或确认通过，不能自动等同于全部业务动作完成。
...
## 8. v2 → v2.1 去 Spike 数据核验

源文件全量差分已完成：1,149 → 1,057 条，精确删除 92 条（8.01%），没有新增或改写保留记录。两版预处理都因超长再丢弃 15 条，Train 为 1,102 → 1,010，Val 均为 32；Val 成员是否相同尚未核对。

按源文件实际工具调用统计，Drive 覆盖为 97.39% → 97.26%，Docs 为 42.12% → 41.53%，Sheets 为 16.88% → 16.27%，Slides 为 10.18% → 10.50%；主要工具分布变化不到 1 个百分点。多工具记录会重复计入各族，不能相加为 100%，也不能把它等同于训练 split 的语义任务配比。

历史说明记录按 assistant content + reasoning_content 的 MD5 定位删除。原始候选选择脚本与 spike 到样本的完整映射仍未找到；本次补齐的是事后整行 SHA-256 差分、92 条删除指纹和聚合分布。详见[去 Spike 实验报告](spike-study/README.md)。

## 9. 待进一步核验

- 构建记录给出的老合成 PASS 池 681 条 + 新缺口轨迹 468 条，仍需与最终样本逐条关联；完整任务配比、线上 trace → seed → 执行 → 判官的来源链仍待核对。
- 各类成功轨迹的人工抽审结果。
- 合成与真实数据的配比消融。
