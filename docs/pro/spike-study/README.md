...
## 0. 这批数据怎样构造，再进入去 Spike 实验

完整过程是：**线上日志提取用户原话 → 按 trace_id 补回历史 → 改写成独立 query → 构造文件环境和验收目标 → DeepSeek 重新执行 → 判官 PASS 与格式清理 → v2 → 去 Spike 得到 v2.1。**

构建记录中的 v2 为老合成 PASS 池选出的 681 条 + 新缺口轨迹 468 条，共 1,149 条。新缺口任务会复用已有环境、改变 query 和验收目标，再重新执行；不是每条训练样本都对应一条线上失败请求，也不是直接改写旧失败回答。

改写和环境生成使用 Claude Opus 5，执行使用 DeepSeek-V4-Flash-0731 在线版，判官使用 GPT-5.5 + 综合评估 Prompt v61。PASS 是最终任务判定，不保证每一步最优；去 Spike 是在这批已构造的数据上再删除 92 条，不等于过滤判官 FAIL。

字段转换、一个贯穿例子、成功轨迹的含义及 v2 补缺口方式，见[完整数据构造流程](../data-construction.md)。本节是基于构建记录与本地代码的链路核查；下文的源数据差分是另行完成的全量产物核验，不代表逐样本来源链已全部恢复。
